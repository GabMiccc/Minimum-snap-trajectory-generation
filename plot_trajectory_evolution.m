function plot_trajectory_evolution(waypoints, times_init, c_init, times_final, c_final, config)
    ...
    for i = 1:length(t_query_init)
        pos_init(i, :) = sample_pos(t_query_init(i), times_init, c_init, config.n_pos);
    end
    ...
    for i = 1:length(t_query_final)
        pos_final(i, :) = sample_pos(t_query_final(i), times_final, c_final, config.n_pos);
    end
...
function pos = sample_pos(t, times, c, n_pos)
    % Trova il segmento corretto e valuta il polinomio per x, y, z
    m = length(times) - 1;
    if t >= times(end)
        idx = m; t_eval = times(end);
    else
    ...
    basis = poly_basis(n_pos, 0, t_eval);
